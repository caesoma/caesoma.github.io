...
\begin{align} \\bar{f}(\\mathbf{x_\dagger}) & = (K_f \\otimes \\mathbf{k_\dagger^x})^T (K_f \\otimes K^x)^{-1}\\mathbf{y} \\\\ & = (K_f(Kf)^{-1}) \\otimes ((\\mathbf{k_\dagger^x})^T (K^x)^{-1})\\mathbf{y} \\\\ & = I \\otimes ((\\mathbf{k_\dagger^x})^T (K^x)^{-1})\\mathbf{y} \end{align}
